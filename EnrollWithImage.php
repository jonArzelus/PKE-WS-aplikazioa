	...
	if($_SERVER['REQUEST_METHOD'] == 'GET')  { //get eskaera landu
		...
		if (mysqli_num_rows($emaitza) > 0) {
			...
			if((!$esp_izena==false) && (!$esp_mota==false) && (!$esp_eposta==false) && (!$esp_pass==false) && (!$esp_tel==false)) {
				...
				mysqli_query($db,"INSERT INTO erabiltzaileak (IzenAbizena, erabiltzaileMota, PostaElektronikoa, Pasahitza, TelefonoZenbakia, Espezialitatea, Interesak, Argazkia) VALUES ('$izena', '$mota', '$eposta',SHA('$pass'), '$tel', '$esp', '$interesak', '$argazkia_helbidea')");
			} else {
