	...
	function aukeratuBesteak() {
		if(document.getElementById("esp").value=="besterik") { //ikusi ea aukera bestelakoa den
			...
			if(document.getElementById("espezializazioa")==null){
				...
				var hitzak = document.createTextNode(" ");
				var input1 = document.createElement("input");
				input1.setAttribute("name","espezializazioa");
				input1.setAttribute("class","form-control");
				input1.setAttribute("placeholder","(*) adierazi zein espezialitate egiten ari zaren: ");
...
?>
    <div class="container">
		...
		<div class="panel-heading"><h1>Orrialdean Erregistratu</h1></div>
 	 	<div class="panel-body text-center">
		<div id="gorputza">
		 <form id="erregistro" name="erregistro" method="POST" action="EnrollWithImage.php" enctype="multipart/form-data">
  			
  			<input class="form-control" placeholder="(*) Izen-Abizenak" type="text" name="izen-abizenak" required pattern="([A-Z]{1}[a-z ]{1,})*" title="Izen-abizenak letra larriz hasita" oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')"><br>
			
 			<input class="form-control" placeholder="(*) Posta Elektronikoa" type="email" id="eposta-helbidea" name="eposta-helbidea" required pattern="^[a-z]+[0-9]{3}@ikasle\.ehu\.eu?s$" title="emailak unibertsitatekoa izan behar du." oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')" onchange="emailaKonprobatu(this.value); paspostaKonprobatu(0);"><br>
			
  			<input class="form-control" placeholder="(*) Kodea" type="text" id="kodea" name="kodea" required pattern="[0-9]{4}" title="Sartu erositako kodea" oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')"><br>
			
 			<input class="form-control" placeholder="(*) Pasahitza" type="password" name="pasahitza" id="pasahitza" required pattern=".{6,}$" title="6 karaktereko luzeera izan behar du gutxienez." oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')" onchange="pasahitzaKonprobatu(this.value); paspostaKonprobatu(2);"><br>
 			
 			<input class="form-control" placeholder="(*) Pasahitza errepikatu" type="password" name="pasahitza2" id="pasahitza2" required pattern=".{6,}$" title="6 karaktereko luzeera izan behar du gutxienez." oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')" onchange="pasahitzaBerdinak();paspostaKonprobatu(3);"><br>
			
 			<input class="form-control" placeholder="(*) Telefono zenbakia" type="text" name="telefono-zenbakia" required pattern="[0-9]{9}" title="9 zenbakiko telefono zenbakia idatzi mesedez." oninvalid="this.setCustomValidity('Atal hau ezin da hutsik utzi')"><br>
			(*) Espezialitatea:
 			<select class="selectpicker" id="esp" name="espezialitatea" onchange="aukeratuBesteak()" required>
			...
			<p id="parrafoa">Interesa duzun teknologiak eta erremintak:</p><br>
 			<textarea class="form-control" name="interesak" id="interesa" rows="3" cols="30"></textarea> <br><br>
			Sartu argazki bat:<br>
			<input style="display:inline;" id="argazki-fitxategia" type="file" name="argazki-fitxategia" accepts="image/*" onchange="return argazkiaKargatu()" value="Bidali"><br>
			 <img id="argazkia" style="width:150px; height: 100px;" class="argazkia"/> <br>
			 <input class="btn btn-primary" id="botoia" type="submit" name="button" value="Bidali" disabled>
			<input class="btn btn-primary" type="reset" name="button" value="Ezeztatu"> <br><br>
		</form> 
